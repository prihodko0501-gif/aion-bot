...
# (опционально) ключ для админ-эндпоинтов /health, /dbfix
...
def compute_biotime_from_payload(p: dict):
    ...
    biotime = round(
        (sleep_score * 0.6 + recovery * 0.8 - stress * 0.7)
        + 6.0
        - pressure_penalty
        - drop_penalty
        - risk_penalty,
...
# =========================
# APRO (PRO MODE) — ЗАФИКСИРОВАННАЯ ФОРМУЛА
...
def dbfix():
    ...
    try:
        # На всякий случай: гарантируем таблицы (и поле JSONB)
        init_db()
