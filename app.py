...
def set_state(chat_id: int, ui_message_id=None, step=None, payload=None):
    ...
    # COALESCE: не перетираем, если None
    db_exec(
        """
...
def api_post(method: str, payload: dict, timeout: int = 12):
    ...
    try:
        ...
        if r.status_code == 200:
            return data
        # логируем, но не ломаем работу
        print("TG ERROR", r.status_code, (data or r.text)[:500] if isinstance((data or r.text), str) else data)
        return data
    except Exception as e:
        print("TG EXC", repr(e))
        return None
...
def edit_message(chat_id: int, message_id: int, text: str, reply_markup=None) -> bool:
    """
    Возвращает True если edit успешен.
    Обрабатывает TG 400 "message is not modified" как OK.
    Если message_id умер/удалён — вернёт False, чтобы мы создали новое UI сообщение.
    """
...
def hide_bottom_panel_silently(chat_id: int):
    # скрывает reply keyboard (если вдруг был). inline keyboard это не трогает.
...
def ensure_ui(chat_id: int):
    ...
    - если edit не удался (сообщение удалено/не найдено), отправляем новое и сохраняем id
    """
    st = get_state(chat_id)
...
def webhook():
    ...
    if "callback_query" in update:
        ...
        # всегда сохраняем ui_message_id как текущее сообщение интерфейса
    ...
    # любой другой текст: не трогаем (не будем спамить меню), но если UI не существует — создадим
    ensure_ui(chat_id)
    return jsonify({"ok": True}), 200
